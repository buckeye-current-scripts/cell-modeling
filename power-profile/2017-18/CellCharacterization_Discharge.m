...
% Import Data File
test_name = 'PPIHC_A123_P0_1';



fid = fopen(strcat('/Users/krisliu/Documents/OSU/Buckeye Current/', ... 
    '2017-2018/Cell Testing/Power Profile/A123/', ... 
...
% Convert data to that it stops at 55 degree temperature 
if(find(test_temp == 55))
    index = find(test_temp == 55);
    test_time = test_time(1:index);
    test_p = test_p(1:index);
    test_i = test_i(1:index);
    test_v = test_v(1:index);
    test_temp = test_temp(1:index);
end


if (max(test_step == 5) ~= 1)
    ...
    xlim([0 max(test_time)]);
    
    fprintf('\nThe Cell test is %s', test_name);
    ...
    Ah = 0;
    for i = 2:length(test_time)
        Ah = Ah - test_i(i) * (test_time(i) - test_time(i-1)) / 3600;
    end
    fprintf('\nThe Amp Hour is %.4f Ah.', Ah);
    ...
    hold on;
    yyaxis left;
    plot(test_time2, -test_p, 'LineWidth', 1.5);
    plot(test_time2, test_temp, '-r', 'LineWidth', 2);
    
    yyaxis right;
    plot(test_time2, test_v, 'LineWidth', 1.5);
    legend('Power', 'Temperture', 'Voltage');
    hold off;
    
    ax1 = subplot(4,1,1);
    plot(test_time2, -test_p, 'LineWidth', 1);
    title('Power');
    ylabel('Watts');
    
    ax2 = subplot(4,1,2);
    plot(test_time2, test_temp, 'LineWidth', 2);
    title('Temperature');
    ylabel('°C');
    
    ax3 = subplot(4,1,3);
    plot(test_time2, test_v, 'LineWidth', 2);
    title('Voltage');
    ylabel('V');
    
    ax4 = subplot(4,1,4);
    plot(test_time2, test_i, 'LineWidth', 2);
    title('Current');
    xlabel('Time(s)');
    ylabel('Amp');
    linkaxes([ax1, ax2, ax3, ax4], 'x');
    xlim([0 max(test_time2)]);
    ...
    Ah = 0;
    for i = 2:length(test_time2)
        Ah = Ah - test_i(i) * (test_time2(i) - test_time2(i-1)) / 3600;
    end
    fprintf('\nThe Amp Hour is %.4f Ah.', Ah);
end
...
% % Write CSV files
% csvwrite('P0_NCR18650GA_200p.csv',power_cell1);
% csvwrite('P0_INR18650MH1_200p.csv',power_cell1);
% csvwrite('P0_IMR18650_200p.csv',power_cell1);
% csvwrite('P0_INR18650MJ1_200p.csv',power_cell2);
% csvwrite('P0_18650HG2_200p.csv',power_cell3);
% csvwrite('P0_18650MG1_200p.csv',power_cell4);
% csvwrite('P0_NCR20700B_200p.csv',power_cell5);
% csvwrite('P0_NCR26650A_200p.csv',power_cell6);
% csvwrite('P0_IMR26650_200p.csv',power_cell7);
% csvwrite('P0_LFP26650P_200p.csv',power_cell8);
% csvwrite('P0_LFP26650EV_200p.csv',power_cell8);
